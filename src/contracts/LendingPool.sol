...
contract LendingPool is ReentrancyGuard, Ownable {
    ...
    function deposit(address asset, uint256 amount) external nonReentrant {
        ...
        if (position.active) {
            // Accrue interest before adding new deposit
            uint256 interest = calculateLendingInterest(msg.sender, asset);
        ...
        position.interestRate = calculateLendingRate(asset);
    ...
    function withdraw(address asset, uint256 amount) external nonReentrant {
        ...
        uint256 interest = calculateLendingInterest(msg.sender, asset);
        uint256 totalAvailable = position.amount + interest;
    ...
    function borrow(address asset, uint256 amount, uint256 collateralAmount) external payable nonReentrant {
        ...
        if (position.active) {
            uint256 interest = calculateBorrowingInterest(msg.sender, asset);
            position.borrowedAmount += amount + interest;
        ...
        position.interestRate = calculateBorrowingRate(asset);
        position.timestamp = block.timestamp;
    ...
    function repay(address asset, uint256 amount) external nonReentrant {
        ...
        position.borrowedAmount = totalOwed - amount;
        position.timestamp = block.timestamp;
    ...
    function liquidate(address borrower, address asset) external nonReentrant {
        ...
        uint256 healthFactor = calculateHealthFactor(borrower, asset);
        require(healthFactor < 10000, "Position is healthy");
        
        uint256 interest = calculateBorrowingInterest(borrower, asset);
        uint256 totalDebt = position.borrowedAmount + interest;
        
        // Transfer debt from liquidator
        IERC20(asset).transferFrom(msg.sender, address(this), totalDebt);
        
        // Transfer collateral to liquidator (with 5% bonus)
        uint256 liquidationReward = (position.collateralAmount * 10500) / 10000;
        
        totalBorrowed[asset] -= position.borrowedAmount;
